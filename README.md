...
每个请求均可以自定义`姓名`，`学院`，`学号`，`出入校类型` 或使用随机生成的信息。

> 若你打算使用本工具，请了解：使用者本人将自行负责产生的所有后果，本人（开发者）不为任何人使用造成的任何后果负责。
...
2. 参考下面的 API 说明，填好信息（你也可以不填，那会使用随机值，跳至第 `6` 步）。在电脑浏览器（Chromium 内核）上打开你填好信息的URL（我们要把中文 escape 掉，不然微信不认）
